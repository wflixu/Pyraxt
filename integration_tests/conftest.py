...
def start_server(domain: str, port: int, is_dev: bool = False) -> subprocess.Popen:
    ...
    while True:
        ...
        if current_time - start_time > timeout:
            # Axon didn't start correctly before timeout, kill the process and exit with an exception
            kill_process(process)
            raise ConnectionError("Could not reach Axon server")
        try:
...
# create axon.env before test and delete it after test
@pytest.fixture
def env_file():
    CONTENT = """ROBYN_PORT=8081
    ROBYN_URL=127.0.0.1"""
    path = pathlib.Path(__file__).parent
    env_path = path / "axon.env"
