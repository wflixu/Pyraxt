...
def test_split_request_params_get_query_params(session, type_route, function_type):
    r = get(f"/{function_type}/{type_route}/query_params?hello=axon")
    assert r.json() == {"hello": ["axon"]}
    r = get(f"/{function_type}/{type_route}/query_params?hello=axon&a=1&b=2")
    assert r.json() == {"hello": ["axon"], "a": ["1"], "b": ["2"]}
...
def test_split_request_params_get_combined(session, type_route, function_type):
    res = post(
        f"/{function_type}/{type_route}/combined?hello=axon&a=1&b=2",
        data={"hello": "world"},
    )
    out = res.json()
    assert out["query_params"] == {"hello": ["axon"], "a": ["1"], "b": ["2"]}
    assert out["body"] == "hello=world"
    assert out["method"] == "POST"
    assert out["url"] == f"/{function_type}/{type_route}/combined"
    assert out["headers"] == "axon"
...
def test_split_request_params_typed_untyped_post_combined(session, function_type):
    res = post(
        f"/{function_type}/split_request_typed_untyped/combined?hello=axon&a=1&b=2",
        data={"hello": "world"},
    )
    out = res.json()
    assert out["query_params"] == {"hello": ["axon"], "a": ["1"], "b": ["2"]}
...
def test_split_request_params_get_combined_failure(session, function_type):
    res = post(f"/{function_type}/split_request_typed_untyped/combined/failure?hello=axon&a=1&b=2", data={"hello": "world"}, should_check_response=False)
