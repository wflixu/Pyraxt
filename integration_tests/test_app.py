...
from axon import ALLOW_CORS, Axon
from axon.events import Events
from axon.axon import Headers


@pytest.mark.benchmark
def test_add_request_header():
    app = Axon(__file__)
    app.set_request_header("server", "axon")
    assert app.request_headers.get_headers() == Headers({"server": "axon"}).get_headers()
