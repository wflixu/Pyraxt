...
from axon import status_codes
from axon.argument_parser import Config
from axon.authentication import AuthenticationHandler
from axon.dependency_injection import DependencyMap
from axon.env_populator import load_vars
from axon.events import Events
from axon.jsonify import jsonify
from axon.logger import Colors, logger
from axon.openapi import OpenAPI
from axon.processpool import run_processes
from axon.reloader import compile_rust_files
from axon.responses import html, serve_file, serve_html
from axon.axon import FunctionInfo, Headers, HttpMethod, Request, Response, WebSocketConnector, get_version
from axon.router import MiddlewareRouter, MiddlewareType, Router, WebSocketRouter
from axon.types import Directory
from axon.ws import WebSocket
...
class BaseAxon(ABC):
    """This is the python wrapper for the Axon binaries."""
    ...
    def _handle_dev_mode(self):
        ...
        is_axon = os.getenv("ROBYN_CLI", False)

        if cli_dev_mode and not is_axon:
            raise SystemExit("Dev mode is not supported in the python wrapper. Please use the Axon CLI. e.g. python3 -m axon app.py --dev")

        if env_dev_mode and not is_axon:
            logger.error("Ignoring ROBYN_DEV_MODE environment variable. Dev mode is not supported in the python wrapper.")
            raise SystemExit("Dev mode is not supported in the python wrapper. Please use the Axon CLI. e.g. python3 -m axon app.py")
    ...
    def include_router(self, router):
        ...
        :param router Axon: the router object to include the routes from
        """
...
class Axon(BaseAxon):
    def start(self, host: str = "127.0.0.1", port: int = 8080, _check_port: bool = True):
        ...
        logger.info("Axon version: %s", __version__)
...
def ALLOW_CORS(app: Axon, origins: Union[List[str], str]):
    """
    Configure CORS headers for the application.

    Args:
        app: Axon application instance
