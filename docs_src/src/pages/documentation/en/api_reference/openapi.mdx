...
After deploying the application, Batman got multiple queries from the users on how to use the endpoints. Axon showed him how to generate OpenAPI specifications for his application.
...
To use a custom openapi configuration, you can:
    ...
    - Or, pass the file path to the `openapi_file_path` parameter in the `Axon()` constructor. (the parameter gets priority over the file).
...
```python {{ title: 'untyped' }}
from axon.types import JSONResponse, Body
...
With the reference documentation deployed and running smoothly, Batman had a powerful new tool at his disposal. The Axon framework had provided him with the flexibility, scalability, and performance needed to create an effective crime-fighting application, giving him a technological edge in his ongoing battle to protect Gotham City.


## What's next?


Batman wondered about whether Axon handlers can be dispatched to multiple processes.

Axon showed him the way!
