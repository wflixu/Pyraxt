...
The first thing you need to is to create a Rust file. Let's call it `hello_world.rs`. You can do it using the cli:
  ...
  <Col sticky>
    ...
    python -m axon --create-rust-file hello_world
    ```
...
<Row>
  ...
  <Col sticky>
    <CodeGroup title="Request" tag="GET" label="/hello_world">
      ...
      ```python {{ title: 'untyped' }}
      python -m axon --compile-rust-path "." --dev
      ```
...
An example of a Axon app with a Rust file that using the `rusqlite` crate to connect to a database and return the number of rows in a table: https://github.com/sansyrox/rusty-sql


## What's next?


Batman was curious to know what else he could do with Axon. 

Axon told him to keep an eye on the GraphQl support.
