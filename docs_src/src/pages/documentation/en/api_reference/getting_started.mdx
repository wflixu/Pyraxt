...
To begin, Axon taught Batman about its ability to handle both synchronous and asynchronous requests. Batman was excited to learn about these features and started implementing them in the application.
...
Batman was curious about how to run the application. Axon explained that he could run the application through a simple command `python3 app.py`


<Row>
  <Col>
    A axon application exposes certain commands to the user. These commands can be used to run the application, or to generate a new project.
  </Col>
  <Col sticky>
    <CodeGroup title="Request" tag="GET" label="/hello_world">
      ```bash
...
<Row>
  <Col>
    Or the application can be run using Axon's CLI, i.e. using `python -m axon app.py`
  </Col>
  ...
  <Col sticky>
    <CodeGroup title="Request" tag="GET" label="/hello_world">
      ...
      usage: python -m axon app.py [-h] [--processes PROCESSES] [--workers WORKERS] [--dev] [--log-level LOG_LEVEL] [--create] [--docs] [--open-browser] [--version]
...
Axon then taught Batman how to handle various HTTP requests like GET, POST, PUT, PATCH, and DELETE. With Axon's guidance, Batman could create endpoints for each request type, making the application versatile and efficient.
...
Batman was curios about the ability to return JSON responses from the application. Axon showed him how to do this using the `jsonify` function.
...
<Col>
  ...
  <Col sticky>
    ...
    ```python {{title: 'typed'}}
    from axon import jsonify, Request
...
<Row>
  ...
  Axon also showed Batman how to access path parameters and query parameters from incoming requests.
  ...
  <Col sticky>
    ...
    ```python {{ title: 'typed' }}
    from axon import jsonify
    from axon.types import PathParams
...
<Row>
  ...
  <Col sticky>
    ...
    ```python {{ title: 'typed' }}
    from axon import Request
    from axon.axon import QueryParams
...
<Row>
 ...
 Do note that the type annotations will take precedence over the reserved names.
  ...
  Axon showed Batman example syntaxes of accessing the request params:
  ...
  <Col sticky>
    ...
    ```python
    from axon.axon import QueryParams, Headers
    from axon.types import PathParams, RequestMethod, RequestBody, RequestURL
...
As Batman continued to develop his web application with Axon, he explored more features and implemented them using code samples.

## Customizing Response Formats and Headers


After understanding the dynamic nature of Axon, Batman, now wanted the ability to customize response formats and headers. Axon showed him how to do this using dictionaries and Axon's Response object.

### Using Dictionaries

<Row>
<Col>
Batman learned to customize response formats by returning dictionaries or using Axon's Response object. He could also set status codes and headers for each response. For example, Batman created a response with a dictionary like this:
</Col>
  <Col sticky>
...
To use the Response object, he wrote:
  ...
  <Col sticky>
    ...
    ```python {{ title: 'untyped' }}
    from axon.axon import Response
    ...
    ```python {{title: 'typed'}}
    from axon.axon import Response, Request
...
Batman then wanted to return a binary output from his application. He could do this by setting the type of the response to "binary" and returning a bytes object. For example, he wrote:
  ...
  <Col sticky>
    ...
    ```python {{title: 'typed'}}
    from axon import Request, Response
...
Batman, being the world's greatest detective, spotted the `headers` field in the `Response` object. He, naturally wanted to know more about it. Axon explained that he could use the `headers` field to set response headers. For example, he could set the `Content-Type` header to `application/json` by writing:
...
Batman, now wanted to know how to read request headers. Axon explained that he could use the `request.headers` field to read request headers. For example, he could read the `Content-Type` header by writing:
...
<Col sticky>
    ...
    ```python {{ title: 'untyped' }}
    app.add_request_header("server", "axon")
    ```
...
After learning about response formats and headers, Batman learned to set status codes for his responses. 
  ...
  <Col sticky>
    ...
    ```python {{title: 'typed'}}
    from axon import status_codes, Request
...
Great, now Axon, what is the `Request` Object that you keep talking about?, Batman said. "Next section", said Axon.

- [The Request Object](/documentation/api_reference/request_object)

Batman was also interested to know about the architecture of Axon. "Next section", said Axon.
