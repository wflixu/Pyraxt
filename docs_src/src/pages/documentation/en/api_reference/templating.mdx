...
Batman wanted to quickly render html pages on the website. He wanted to use a templating engine to render the html pages. Axon told him that he can use the Jinja2 templating engine to render the html pages. He can use the `JinjaTemplate` class to render the html pages.
...
Batman was also super excited to know that Axon allows the support of custom templating engines.

<Row>
<Col>
To do that, you need to import the `TemplateInterface` from `axon.templating`
</Col>
  <Col sticky>
    <CodeGroup title="Request" tag="GET" label="/hello_world">

      ```python {{ title: 'untyped' }}
      from axon.templating import TemplateInterface
      ```
