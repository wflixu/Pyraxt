...
/// Shared application state passed to Axum handlers via State extractor
#[derive(Clone)]
struct AppState {
...
impl Server {
    ...
    pub fn start(
        ...
        thread::spawn(move || {
            ...
            runtime.block_on(async move {
                ...
                let state = AppState {
                    router: router.clone(),
                    const_router: const_router.clone(),
                    middleware_router: middleware_router.clone(),
                    global_request_headers: global_request_headers.clone(),
                    global_response_headers: global_response_headers.clone(),
                    excluded_response_headers_paths: excluded_response_headers_paths.clone(),
                };

                let mut app: AxumRouter<()> = AxumRouter::new();

                // Static file serving
                let directories = directories.read().unwrap();
                for directory in directories.iter() {
                    let service = if directory.index_file.is_some() {
                    ...
                    app = app.nest_service(&directory.route, service);
                }

                // WebSocket routes
                let web_socket_map = web_socket_router.get_web_socket_map();
                for (endpoint, value) in web_socket_map.read().iter() {
                    let endpoint_owned = endpoint.clone();
                    let path_params = value.clone();
                    let ws_task_locals = task_locals_copy.clone();
                    let endpoint_ref = endpoint_owned.clone();
                    app = app.route(
                        &endpoint_ref,
                        get(move |ws| {
                            let endpoint = endpoint_owned.clone();
                            let path_params = path_params.clone();
                            let task_locals = ws_task_locals.clone();
                            async move {
                                start_web_socket(ws, path_params, task_locals, endpoint, None).await
                            }
                        }),
                    );
                }
                ...
                // Main fallback handler for all HTTP requests
                // We use a closure to capture state, avoiding Axum State which
                // would make the Router not implement Service<IncomingStream>
                let handler_state = state.clone();
                app = app.fallback(move |req: AxumRequest| {
                    let state = handler_state.clone();
                    async move {
                        // Extract parts before consuming the body
                        let method = HttpMethod::from_http_method(req.method());
                        ...
                        let mut request =
                            Request::from_axum_request(http_req, &state.global_request_headers).await;

                        // Before middleware
                        let mut before_middlewares = state
                            .middleware_router
                            .get_global_middlewares(&MiddlewareType::BeforeRequest);
                        if let Some((function, route_params)) =
                            state
                                .middleware_router
                                .get_route(&MiddlewareType::BeforeRequest, &path)
                        {
                        ...
                        for before_middleware in before_middlewares {
                            request =
                                match execute_middleware_function(&request, &before_middleware)
                                    .await
                                {
                                    ...
                                    Err(e) => {
                                        ...
                                        return Response::internal_server_error(None).into_response();
                                    }
                                };
                        }
                        ...
                        // Route execution
                        let mut response =
                            if let Some(res) = state.const_router.get_route(&method, &path) {
                                res
                            } else if let Some((function, route_params)) =
                                state.router.get_route(&method, &path)
                            {
                                request.path_params = route_params;
                        ...
                        response.headers.extend(&state.global_response_headers);

                        match &state.excluded_response_headers_paths {
                            None => {}
                            Some(excluded_response_headers_paths) => {
                                if excluded_response_headers_paths.contains(&path) {
                ...
                app = app.layer(RequestBodyLimitLayer::new(
                    max_payload_size * 2, // Allow some overhead
                ));

                // Create TCP listener from shared socket
                let std_listener: std::net::TcpListener = raw_socket.into();
                let listener = tokio::net::TcpListener::from_std(std_listener).unwrap();

                debug!("Server starting on listener");
    ...
    /// can be called after the server has started
    pub fn add_route(
        &self,
        py: Python,
